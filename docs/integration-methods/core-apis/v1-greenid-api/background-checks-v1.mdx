---
id: background-checks-v1
title: Background Checks V1
sidebar_position: 9
---

# Background Checks V1
...
- passportGenderCode - the person's gender.  This field is optional and you need to let us know if you want it to count towards the passport check.  Valid values are "M" (male") or "F" (female).  This field must have the value as shown on the passport.
...
- passportExpiryDate - the expiry date, exactly as it appears on the document.  This field is required.  This field must have the value as shown on theNZ issued passport.
...
The result of the background driver's licence check will be returned in the standard [RichVerificationResult](https://vixverify.atlassian.net/wiki/spaces/GREEN/pages/8880477/Data+Structures#DataStructures-RichVerificationResult) element returned by the web service call (either the regular greenID web services, or the greenID API).
